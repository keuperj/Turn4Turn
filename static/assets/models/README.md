...
Both packs are CC0; original licenses are preserved as `static/scenarios/farm/models/LICENSE-farm-vehicles.txt`
and `static/scenarios/farm/models/LICENSE-farm-animals.txt`. The animal download is also available on
...
The farm manifest records per-model authors, sources, dimensions and SHA-256 hashes.
...
### Airport aircraft and ground support

Six models from [Aircraft Fleet and Airfield](https://3dassets.dev/packs/aircraft-fleet-and-airfield)
by 3DAssets.dev are bundled under CC0 1.0: a high-wing light plane, light business
jet, aviation fuel bowser, aircraft tug, ground power cart, and windsock.
The source publisher identifies these models as AI-generated. Individual source
and download URLs, original and bundled SHA-256 hashes, and normalized dimensions
are recorded in `static/scenarios/airport/models/manifest.json`; see its `LICENSE-airport.txt`.

Rebuild with `.venv/bin/python tools/import_airport.py /tmp/airport-sources`
(requires curl, Playwright and Chromium). The importer preserves materials and
exports static parked poses at a centered, grounded origin. Aircraft use a compact
tactical scale, uniformly reduced further when needed to fit smaller maps.
Terminal, tower and hangars use the game's enterable building geometry so their
walls, doors, floors and cutaways agree with the simulation. No runtime CDN is used.

### Factory machinery

Eight CC0 models are bundled from 3DAssets.dev's
[Machine Shop and Factory Hall](https://3dassets.dev/packs/machine-shop-and-factory-hall)
and [Car Factory Production Line](https://3dassets.dev/packs/car-factory-production-line):
lathe, mill, CNC machining centre, compressor, storage rack, forklift, welding robot
and conveyor. The publisher identifies these models as AI-generated.
Source URLs, SHA-256 hashes, dimensions and licensing are recorded in `static/scenarios/factory/models/manifest.json`
and its `LICENSE-factory.txt`.

Rebuild with `.venv/bin/python tools/import_factory.py /tmp/factory-sources`
(requires curl, Playwright and Chromium). Models retain their materials, are exported
in static poses, and are uniformly normalized to their collision footprints and a
maximum height of 2.5 m, fitting beneath the three-metre floor spacing. Platforms,
stairs, guardrails and the two-wall hall shell are generated by the game.

## Scenario-owned packs

Farm, airport, factory and rail models now live under
`static/scenarios/{farm,airport,factory,train_station}/models/`, each with its own
manifest and licenses. Shared road vehicles remain in `transport/`.
`/api/models` exposes the combined catalog. See [scenario documentation](../../../docs/scenarios.md)
for addon and import-tool instructions.
