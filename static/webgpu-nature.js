...
function natureMaterials(scene){
 ...
 const birch=new B.PBRMaterial('birch-bark',scene);birch.albedoColor=B.Color3.FromHexString('#e4e0ce');birch.albedoTexture=qualityMaterials(scene).stone;birch.bumpTexture=qualityMaterials(scene).normal;birch.roughness=1;birch.metallic=0;
 const value={bark,birch,leaves};cache.set(scene,value);return value;
}
export function detailedTree(scene,parent,pos,shadows,scale=1,variant=0,species='tree_oak'){
 const materials=natureMaterials(scene),pine=species==='tree_pine',birch=species==='tree_birch',root=new B.TransformNode('detailed-tree',scene);root.parent=parent;root.position.copyFromFloats(...pos);root.scaling.setAll(scale);
 const branches=[],leaves=[];
 function branch(a,b,r1,r2){const p=new B.Vector3(...a),q=new B.Vector3(...b),d=q.subtract(p),m=B.MeshBuilder.CreateCylinder('branch',{height:d.length(),diameterBottom:r1*2,diameterTop:r2*2,tessellation:10},scene);m.position=p.add(q).scale(.5);m.rotationQuaternion=B.Quaternion.FromUnitVectorsToRef(B.Axis.Y,d.normalize(),new B.Quaternion());m.material=birch?materials.birch:materials.bark;branches.push(m);}
 branch([0,0,0],[.10,3.5,0],.19,.075);
 for(let i=0;i<12;i++){
  const a=i*2.39996+variant,h=pine?.65+i*.25:1.6+(i%5)*.35,r=pine?1.35-i*.085:birch?.60+(i%3)*.13:1.05+(i%3)*.21,end=[Math.sin(a)*r,h+.8,Math.cos(a)*r];branch([0,h,0],end,.065,.014);
  for(let j=0;j<3;j++){const t=[end[0]+Math.sin(a+j)*.4,end[1]+.2+j*.16,end[2]+Math.cos(a+j)*.4];branch(end,t,.022,.006);
   for(let k=0;k<8;k++){const phi=k*2.4+j,radius=.18+(k%3)*.15,m=B.MeshBuilder.CreatePlane('leaf-spray',{size:.62+(k%3)*.10,sideOrientation:B.Mesh.DOUBLESIDE},scene);m.position.copyFromFloats(t[0]+Math.sin(phi)*radius,t[1]+Math.cos(phi)*.36,t[2]+Math.cos(phi)*radius);m.rotation.set(k*.83+j,k*1.7+variant,j*.8);m.material=materials.leaves;if(pine)m.scaling.set(.32,1.2,1);leaves.push(m);}
  }
 }
...
export function detailedEffects(scene,ambient=false){
 const tex=new B.DynamicTexture('turbulent-smoke',smokeCanvas(),scene,true);tex.hasAlpha=true;tex.update(false);
 const particles=Array.from({length:ambient?54:148},(_,i)=>{
  const kind=ambient?(i<36?'smoke':'fire'):i<72?'smoke':i<90?'fire':i<118?'spark':i<132?'debris':'dust';
 ...
 return {particles,update(event,origin=[2.5,.2,3],size=1){
  const age=event?.age??9,blast=event?.kind==='explosion',continuous=event?.kind?.startsWith('continuous-'),burning=event?.kind==='continuous-fire';light.position.copyFromFloats(origin[0],origin[1]+1,origin[2]);light.intensity=burning?.7+Math.sin(age*9)*.15:blast?Math.max(0,1-age/.35)*7:0;
  particles.forEach(({m,kind},i)=>{const a=i*2.39996,delay=kind==='smoke'?(i%12)*.06:0,lifetime=kind==='smoke'?7:kind==='fire'?.75:kind==='spark'?1.4:kind==='dust'?2.5:2.2,t=continuous?(age+i*.379)%lifetime:age-delay;
   const enabled=!!event&&t>=0&&t<lifetime&&(blast||kind==='smoke'||burning&&kind==='fire');m.setEnabled(enabled);if(!enabled)return;
   let x,y,z,s,opacity;
   if(kind==='smoke'){const r=(.2+(i%9)*.10)*(1+t*.24),wind=t*.13;x=Math.cos(a)*r+wind;y=.22+t*(.24+(i%5)*.035)+(i%6)*.18;z=Math.sin(a)*r;s=.65+t*.28+(i%4)*.12;opacity=Math.min(1,t*3)*Math.pow(1-t/lifetime,1.3)*.62;m.material.emissiveColor.copyFromFloats(blast||burning?.29:.57,blast||burning?.30:.60,blast||burning?.31:.62);}
 ...
 },dispose(){for(const {m} of particles){const material=m.material;m.dispose();material.dispose(false,false);}tex.dispose();light.dispose();}};
}
