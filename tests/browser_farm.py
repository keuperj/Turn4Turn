"""Farm assets, vegetation and retained scenery during fog updates."""
import argparse
import sys
import threading
...
def main(base,gpu):
    with sync_playwright() as p:
        ...
        page.route('**/farm-test',lambda route:route.fulfill(content_type='text/html',body='<body style="margin:0"><div style="width:1000px;height:750px"><canvas style="width:100%;height:100%"></canvas></div></body>'))
        page.goto(base+'/farm-test');page.add_script_tag(url='/vendor/babylon.js');page.add_script_tag(url='/vendor/babylonjs.loaders.min.js')
        ...
        for size in (24,30,40):
            g=Game(41,'farm',size=size);state=g.state()
            ...
            result=page.evaluate('''state=>{
              ...
              const outside=meshes.filter(m=>m.metadata?.asset).every(m=>{m.computeWorldMatrix(true);const b=m.getBoundingInfo().boundingBox;return b.maximumWorld.x<-.5||b.maximumWorld.z<-.5||b.minimumWorld.x>n-.5||b.minimumWorld.z>n-.5;});
              const farmOwners=f.pickables.filter(p=>p.userData?.transport?.startsWith('Farm'));
              const modelsFit=farmOwners.every(owner=>{
              ...
              return {size:n,buildings:state.buildings.length,plants:state.props.length,outside,modelsFit,
                backgroundPlants:f.background.placements.filter(p=>/Tree|Bush/.test(p.id)).length,
                grass:f.nativeScene.getMeshByName('farm-pasture-grass')?.getTotalVertices()>0,
                farmModels:[...new Set(f.pickables.map(p=>p.userData?.transport).filter(id=>id?.startsWith('Farm')))],
            ...
            assert all(result[k] for k in ('outside','isolated','retained','modelsFit')),result
            assert result['backgroundPlants']>150 and result['grass'],result
            assert set(result['farmModels'])=={'FarmTractor','FarmLoader','FarmCow','FarmSheep','FarmPig'},result
            page.wait_for_function('f.nativeScene.isReady()');page.evaluate('f.engine.beginFrame();f.nativeScene.render();f.engine.endFrame()')
            if size==30 and not gpu:page.screenshot(path='/tmp/farm.jpg',type='jpeg',quality=75)
            page.evaluate("f.setView('0');f.sync(f.state,'s0',null,'move');f.engine.beginFrame();f.nativeScene.render();f.engine.endFrame()")
            print(result,flush=True)
        assert not errors,errors
        browser.close()
        print('PASS farm rendering, background bounds, picking isolation and retained fog updates',flush=True)
