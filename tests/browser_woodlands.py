"""Woodland visuals, forest continuation and retained scenery during fog updates."""
import argparse
import sys
import threading
...
def main(base,gpu):
    with sync_playwright() as p:
        ...
        page.route('**/woodland-test',lambda route:route.fulfill(content_type='text/html',body='<body style="margin:0"><div style="width:1000px;height:750px"><canvas style="width:100%;height:100%"></canvas></div></body>'))
        page.goto(base+'/woodland-test');page.add_script_tag(url='/vendor/babylon.js');page.add_script_tag(url='/vendor/babylonjs.loaders.min.js')
        ...
        for size in (24,30,40):
            g=Game(41,'woods',size=size);state=g.state()
            ...
            result=page.evaluate('''state=>{
              ...
              return {size:n,huts:state.buildings.length,plants:state.props.length,outside,
                backgroundPlants:f.background.placements.length,variants:new Set(f.background.placements.map(p=>p.id)).size,
            ...
            assert all(result[k] for k in ('outside','isolated','retained')),result
            assert result['backgroundPlants']>500 and result['variants']==6,result
            page.wait_for_function('f.nativeScene.isReady()');page.evaluate('f.engine.beginFrame();f.nativeScene.render();f.engine.endFrame()')
            if size==30 and not gpu:page.screenshot(path='/tmp/woodland.jpg',type='jpeg',quality=75)
            page.evaluate("f.setView('0');f.sync(f.state,'s0',null,'move');f.engine.beginFrame();f.nativeScene.render();f.engine.endFrame()")
            print(result,flush=True)
        assert not errors,errors
        browser.close()
        print('PASS woodland rendering, background bounds, picking isolation and retained fog updates',flush=True)
