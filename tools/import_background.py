"""Rebuild the bundled Quaternius background scenery from FBX sources using Playwright.

Usage: .venv/bin/python tools/import_background.py /path/to/source-fbx
...
DEST = ROOT / 'static/assets/models/background'
...
def main():
    ...
    if args.download:
        ...
        sources.extend((e['file'],e['source']) for e in manifest['textures'])
    ...
    class Handler(SimpleHTTPRequestHandler):
        ...
        def translate_path(self, path):
            ...
            if path == '/background-convert.js':
                return str(ROOT / 'tools/background-convert.js')
    ...
    try:
        with sync_playwright() as p:
            ...
            for entry in manifest['models']:
                result = page.evaluate("async entry=>(await import('/background-convert.js')).convertBackground(entry)", entry)
