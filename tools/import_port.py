"""Bundle selected CC0 port GLBs, normalized to tactical prop footprints.

Usage: .venv/bin/python tools/import_port.py /tmp/port-sources
...
PACK='https://3dassets.dev/api/v1/packs/marina-and-yacht-club'
SELECTION=[('38285','PortPontoon','port_pontoon',8,2.4),
           ('38307','PortCruiser','port_boat',3.6,9),
           ('38308','PortCruiserNavy','port_boat',3.6,9),
           ('38309','PortTender','port_boat',1.7,4),
           ('38293','PortDockBox','port_box',.9,.9),
           ('38294','PortPedestal','port_pedestal',.6,.6),
           ('38300','PortLamp','port_lamp',.7,.7),
           ('38312','PortHoist','port_hoist',5,5),
           ('38313','PortCrane','port_crane',3,3),
           ('38326','PortBuoy','port_buoy',1.2,1.2),
           ('38329','PortLifebuoy','port_lifebuoy',.7,.7)]


def main():
    source=Path(sys.argv[1]);source.mkdir(parents=True,exist_ok=True)
    dest=ROOT/'static/scenarios/port/models'
    ...
    manifest={'license':'CC0-1.0','packs':{},'models':[]}
    try:
        with sync_playwright() as p:
            ...
            for asset_id,name,kind,w,d in SELECTION:
                ...
                entry=entries[asset_id]
                record=dict(id=name,kind=kind,scenery_only=kind not in ('port_box','port_pedestal'),pack='port',file=name+'.glb',dimensions=result['dimensions'],sha256=hashlib.sha256(data).hexdigest(),sourceSha256=hashlib.sha256((source/(name+'.glb')).read_bytes()).hexdigest(),source=entry['url'],download=entry['cdnUrl'],author='3DAssets.dev',license='CC0-1.0',aiGenerated=entry['aiGenerated'])
                manifest['models']=[e for e in manifest['models'] if e['id']!=name]+[record]
                print(name,len(data),result['dimensions'],flush=True)
            browser.close()
        manifest['packs']['port']='https://3dassets.dev/packs/marina-and-yacht-club'
        (dest/'manifest.json').write_text(json.dumps(manifest,indent=2)+'\n')
        (dest/'LICENSE-port.txt').write_text('Marina and Yacht Club — 3DAssets.dev\nhttps://3dassets.dev/packs/marina-and-yacht-club\n\nCC0 1.0 Universal\nhttps://creativecommons.org/publicdomain/zero/1.0/\n\nThe publisher dedicates these models to the public domain.\nFree to use, modify, and redistribute, including commercially; attribution is not required.\nThe publisher identifies these assets as AI-generated.\nSee manifest.json for individual source URLs and SHA-256 hashes.\n')
